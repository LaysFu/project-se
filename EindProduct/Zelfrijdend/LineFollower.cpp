...
void LineFollower::followLine() {
  ...
  while (true) {
    ...
    } else if (lineSensorValues[2] == Color::Gray) {
    ...
    if (turnLeft) {
      ...
      if (lineSensorValues[2] == Color::Black) {
        turnRight();
        turnRight = false;
      }
    }
...
bool LineFollower::isBrownLineDetected(Color lineSensorValues[3]) {
  return (lineSensorValues[0] == Color::Brown || lineSensorValues[2] == Color::Brown);
}

bool LineFollower::isGrayLinesDetected(Color lineSensorValues[3]) {
  return (lineSensorValues[0] == Color::Gray && lineSensorValues[2] == Color::Gray);
}
...
void LineFollower::updateOnGreenLine(Color lineSensorValues[3]) {
  onGreenLine = (lineSensorValues[1] == Color::Green);
}

int LineFollower::calculateLinePosition(Color lineSensorValues[3]) {
  int position = 0;
  int count = 0;
  for (int i = 0; i < 3; i++) {
